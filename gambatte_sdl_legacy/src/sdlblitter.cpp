...
void SdlBlitter::setBufferDimensions() {
	...
	switch(selectedscaler) {
		...
		case 11:		/* Hardware Fullscreen */
			...
			break;
	    case 12: //HW X2
		case 13: //HW X2 scanline grid
		    SetVid(160<<1, 144<<1, 16);
			if (aspect_ratio_file)
			{
				fwrite("0", 1, 1, aspect_ratio_file);
			}
...
void SdlBlitter::setScreenRes() {
	...
	switch(selectedscaler) {
	    ...
	    case 12:
		case 13:
		    if(screen->w != 160 << 1 || screen->h != 144 << 1)
		        SetVid(160 << 1, 144 << 1, 16);
			if (aspect_ratio_file)
			{
				fwrite("0", 1, 1, aspect_ratio_file);
...
void SdlBlitter::draw() {
	...
	if((ghosting == 0) || ((ghosting == 1) && (gameiscgb == 1)) || ((ghosting == 2) && (gameiscgb == 0))){ //Ghosting disabled for current system
		    ...
		    case 12:/*hardware x2*/
		        {
                    ...
                    gbc_convert_2x(d, s, surface->pitch, screen->pitch);
	            }
				break;
            case 13: /*hardware x2 scanline grid*/
			    {
                    uint16_t *d = (uint16_t*)screen->pixels;
	                uint16_t *s = (uint16_t*)surface->pixels;
                    gbc_convert_2x_scanline_grid(d, s, surface->pitch, screen->pitch);
	            }
				break;
			case 7:		/* Hardware 1.25x */
			case 8:		/* Hardware 1.36x */
...
#endif
			default:
				uint16_t *dr = (uint16_t*)screen->pixels + (screen->w - surface->w ) / 2 + (screen->h - surface->h) * screen->pitch / 4;
	            uint16_t *sr = (uint16_t*)surface->pixels;
	            for (int y = 0; y < surface->h; y++)
	            {
	                memmove(dr, sr, surface->w * sizeof(uint16_t));
	                sr += surface->w;
	                dr += screen->w;
	            }
				break;
		...
		switch(selectedscaler) {
            ...
            {
                //TODO littlehui
                uint16_t *dt = (uint16_t*)screen->pixels + (screen->w - currframe->w) / 2 + (screen->h - currframe->h) * screen->pitch / 4;
                uint16_t *st = (uint16_t*)currframe->pixels;
                for (int y = 0; y < currframe->h; y++)
                {
                    memmove(dt, st, currframe->w * sizeof(uint16_t));
                    st += currframe->w;
                    dt += screen->w;
                }
...
void SdlBlitter::scaleMenu() {
	...
	switch(selectedscaler) {
        ...
        case 12:
                {
                    ...
                    gbc_convert_2x(d, s, menuscreen->pitch, screen->pitch);
	            }
				break;
        case 13:
                {
                    uint16_t *dmenu = (uint16_t*)screen->pixels;
                    uint16_t *smenu = (uint16_t*)menuscreen->pixels;
                    gbc_convert_2x_scanline_grid(dmenu, smenu, menuscreen->pitch, screen->pitch);
	            }
				break;
		case 7:		/* Hardware 1.25x */
		case 8:		/* Hardware 1.36x */
		...
		default:
        {
            //TODO littlehui scala menu
            uint16_t *dmenu = (uint16_t*)screen->pixels + (screen->w - menuscreen->w) / 2 + (screen->h - menuscreen->h) * screen->pitch / 4;
            uint16_t *smenu = (uint16_t*)menuscreen->pixels;
            for (int y = 0; y < menuscreen->h; y++)
            {
                memmove(dmenu, smenu, menuscreen->w * sizeof(uint16_t));
                smenu += menuscreen->w;
                dmenu += screen->w;
            }
        }
