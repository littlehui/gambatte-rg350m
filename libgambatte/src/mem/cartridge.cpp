...
public:
  ...
  ~Mbc5() {
    ...
    case 4:
    case 5:
      if((enableRumble_ == true) && ((p >> 12 & 0x7) == 4)){
        if((data & 8) != preRumbleVal_){
          preRumbleVal_ = data & 8;
          if(preRumbleVal_) {
            shake_it(2);
          }
        }
  ...
  void shake_init(void)
  {
    ...
    if(Shake_NumOfDevices() > 0){
      vibrator_ = Shake_Open(0);
      memset(&effect, 0, sizeof(effect));
      Shake_SimpleRumble(&effect, 50,  0, 0.5); 
      id = Shake_UploadEffect(vibrator_, &effect);
      Shake_Play(vibrator_, id);
      usleep(300000);
      Shake_EraseEffect(vibrator_, id);
    }
  }

  void shake_it(int level)
  {
    int var;
    static int id=-1;
    Shake_Effect effect;
    ...
    if(vibrator_){
      ...
      case 1:
      case 2:
        Shake_SimpleRumble(&effect, 10,  0, 0.1 * var); 
        id = Shake_UploadEffect(vibrator_, &effect);
        Shake_Play(vibrator_, id);
        break;
      case 3:
      case 4:
      case 5:
        Shake_SimpleRumble(&effect, 100,  0, 0.1 * var); 
        id = Shake_UploadEffect(vibrator_, &effect);
        Shake_Play(vibrator_, id);
        usleep(150000);
        Shake_Play(vibrator_, id);
  ...
  void shake_deinit(void)
  {
    if(vibrator_) {
      Shake_Close(vibrator_);
      Shake_Quit();
      vibrator_ = NULL;
    }
  }
...
private:
	...
	bool enableRam_;
  bool enableRumble_;
  unsigned char preRumbleVal_;
  Shake_Device *vibrator_;
                           ...
                           bool const multicartCompat)
{
  bool rumble=false;
...

	{
		...
		case 0x1D:
		case 0x1E: 
      if((header[0x0147] >= 0x1c) && (header[0x0147] <= 0x1e)){
        rumble = true;
      }
      type = type_mbc5; 
      break;
	...
	case type_mbc3:
		...
		break;
	case type_mbc5: mbc_.reset(new Mbc5(memptrs_, rumble)); break;
